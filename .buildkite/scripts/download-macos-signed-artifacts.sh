...
# macos-sign-service is the pipeline step in .buildkite/release.yml
GPG_SIGN_BUILD_ID=$(jq -r '.jobs[] | select(.step_key == "macos-sign-service").triggered_build.id' <<< "$build_json")
...
cat << EOF
  - label: ":pipeline: Download signed artifacts"
    commands:
      ...
      - cd signed-artifacts
      - ls -ltra *.*
      - buildkite-agent artifact upload *.dmg
    agents:
